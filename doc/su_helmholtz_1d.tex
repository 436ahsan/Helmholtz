...
%%%% Low residuals are important.
%% left, right - sin + i*cos, sin - i*cos are the basis functions.
...
%%% generral procedures that arer not only for Helmholtz. THey don't exploit a specific knowledge of the problem like sin and cossine.
%% Importancce of bootstrapping in generating an accurate coarse levels.
...
% Depends on a good smoother. Reduces space. Or can do it from an analysis of the homogenous PDE.
...
%%% Rerun without lambda normalization
...
% Beccause the ampliude of the solution with [x, 0] . Mock cycle is pessimistic. quantitative prediction results from experience. CCould perssimistic butt good in many other examples.
