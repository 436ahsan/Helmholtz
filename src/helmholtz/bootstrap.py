...
def generate_test_matrix(a: scipy.sparse.spmatrix, num_growth_steps: int, growth_factor: int = 2,
    ...
    for i in range(num_bootstrap_steps):
        x, multilevel = bootstap(x, multilevel, aggregate_size=aggregate_size, num_sweeps=num_sweeps)
...
def bootstap(x, multilevel, aggregate_size: int = 4, num_sweeps: int = 10, threshold: float = 0.1, caliber: int = 2,
             interpolation_method: str = "svd") -> Tuple[np.ndarray, hm.multilevel.Multilevel]:
    """
    ...
    for l in range(num_levels):
        ...
        level = create_coarse_level(level, x, aggregate_size=aggregate_size, num_examples=num_examples,
                                    threshold=threshold, caliber=caliber, interpolation_method=interpolation_method)
        new_multilevel.append(level)
        x = level.r.dot(x)
